...
#%%
class GammaAlphaPerPattern_FixScale_NegBinomial_base(PyroModule):
    def __init__(self,
                num_samples,
                num_genes,
                num_patterns,
        ...
        self.loc_A = torch.ones(self.num_patterns, self.num_genes, device=self.device, dtype=default_dtype) * self.scale
        self.loc_P = torch.ones(self.num_samples, self.num_patterns, device=self.device, dtype=default_dtype) * self.scale
