...
class Gamma_NegBinomial_SSFixed(Gamma_NegBinomial_base):
    def __init__(self,
        ...
        self.loc_P = PyroParam(torch.rand(self.num_samples, self.num_fixed_patterns + self.num_patterns, device=self.device), constraint=dist.constraints.positive)        
        self.best_P = torch.zeros(self.num_samples, self.num_fixed_patterns + self.num_patterns, device=self.device)
        self.best_locP = torch.zeros(self.num_samples, self.num_fixed_patterns + self.num_patterns, device=self.device)

        #### Fixed patterns are samples x patterns ####
        self.fixed_A = torch.tensor(fixed_patterns, device=self.device,dtype=torch.float32) # tensor, not updatable
