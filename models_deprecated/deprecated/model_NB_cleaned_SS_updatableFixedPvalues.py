...
class GammaMatrixFactorization(PyroModule):
    def __init__(self,
        ...
        super().__init__()
        print("using Semi-supervised algorithm with updatable 'fixed' patterns")
        self.num_genes = num_genes
        self.num_patterns = num_patterns
        self.fixed_patterns = fixed_patterns # of shape samples x fixed patterns
        if fixed_patterns.shape == (0,):
            self.num_fixed_patterns = 0
        else:
    ...
    def forward(self, D):
        ...
        if self.num_fixed_patterns != 0:
            #with pyro.plate("samples_fixed", self.num_samples, dim=-2):
            #    with pyro.plate("fixed_patterns", self.num_fixed_patterns, dim = -1):
                    #sampled_fixed_P = pyro.sample("fixed P", dist.Gamma(self.fixed_loc_P, self.fixed_scale_P)) # sample P from Gamma
