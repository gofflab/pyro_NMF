...
#%%
class SSgenes_GammaMatrixFactorization(PyroModule):
    def __init__(self,
                 num_genes,
                 num_samples,
                 num_patterns, # number of additional unsupervised patterns
                 fixed_patterns, # fixed_patterns x genes
                 NB_probs = 0.5,
        ...
        self.fixed_patterns = fixed_patterns # of shape fixed_patterns x genes
        self.num_fixed_patterns = fixed_patterns.shape[0]
        ...
        self.loc_P = PyroParam(torch.rand(self.num_samples, self.num_patterns  + self.num_fixed_patterns, device=self.device),constraint=dist.constraints.positive)
        self.scale_P = PyroParam(torch.ones(self.num_samples, self.num_patterns + self.num_fixed_patterns, device=self.device),constraint=dist.constraints.positive)

        self.fixed_A = torch.tensor(fixed_patterns, device=self.device,dtype=torch.float32)
    ...
    def forward(self, D):
        ...
        A_total = torch.cat((self.fixed_A, A), dim=0)
        self.A_total = A_total # save P_total

        # Nested plates for pixel-wise independence?
        with pyro.plate("samples", self.num_samples, dim=-2):
            with pyro.plate("patterns_P", self.num_patterns  + self.num_fixed_patterns, dim = -1):
