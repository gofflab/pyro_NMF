...
class GammaMatrixFactorization(PyroModule):
    ...
    def forward(self, D):
        ...
        # Add L1 (sparsity) and L2 (smoothness) regularization for A
        pyro.factor("A_L1", -self.lambda_A1 * torch.sum(torch.abs(A)))
        pyro.factor("A_L2", -self.lambda_A2 * torch.sum(A ** 2))

        # Add L1 (sparsity) and L2 (smoothness) regularization for P
        pyro.factor("P_L1", -self.lambda_P1 * torch.sum(torch.abs(P)))
        pyro.factor("P_L2", -self.lambda_P2 * torch.sum(P ** 2))
