...
from pyroNMF.models.gamma_NB_new_SSfixedA import Gamma_NegBinomial_SSFixed
import random
...
data = ad.read_h5ad('/mnt/morbo/Users/kwoyshner/data/ABA_MERSCOPE_zeng/Zhuang-ABCA-1-raw_wMeta_wAnnotations_wAtlas_KW.h5ad')
data.obs['brain_section_num'] = data.obs.loc[:,'brain_section_label_x'].apply(lambda x: float(x.split('-')[-1]))
#data = data[data.obs['brain_section_num'].apply(lambda x: x >= 1.043 and x <= 1.062)]   # .apply(lambda x: x >= 1.043 and x <= 1.090)]
data = data[data.obs['brain_section_num'].apply(lambda x: x >= 1.056 and x <= 1.060)]   # .apply(lambda x: x >= 1.043 and x <= 1.090)]
'''

#data = ad.read_h5ad('/mnt/morbo/Users/kwoyshner/data/ABA_MERSCOPE_zeng/Zhuang-ABCA-1-raw_wMeta_wAnnotations_wAtlas_KW.h5ad')
#data = data[data.obs.loc[:,'brain_section_label_x'] == 'Zhuang-ABCA-1.042',:]

data = ad.read_h5ad('/mnt/morbo/Users/kwoyshner/data/ABA_MERSCOPE_zeng/Zhuang-ABCA-1-raw_wMeta_wAnnotations_wAtlas_sub20_KW.h5ad')
...
learned_patterns = ad.read_h5ad('/raid/kyla/projects/pyro_NMF/analyses/base_model_mouse_cortex/fix_layersOnly.h5ad')
#fixed_geneWeights = learned_patterns.varm['best_A'].loc[:,['SS', 'MO']]
fixed_geneWeights = learned_patterns.varm['best_A'].loc[:,['layer 1', 'layer 2/3', 'layer 4', 'layer 5', 'layer 6']]
...
fixed_patterns = torch.tensor(fixed_geneWeights.to_numpy())
fixed_pattern_names = list(fixed_geneWeights.columns)
...
#### output parameters
outputDir = '/raid/kyla/projects/pyro_NMF/analyses/base_model_mouse_cortex'
savename = 'fix_layersOnlyPats_z20' # output anndata will be saved in outputDir/savename.h5ad
...
loc_P.columns = fixed_pattern_names + ['Pattern_' + str(x+1) for x in np.arange(num_patterns)]
...
last_P.columns = fixed_pattern_names + ['Pattern_' + str(x+1) for x in np.arange(num_patterns)]
...
best_P.columns = fixed_pattern_names + ['Pattern_' + str(x+1) for x in np.arange(num_patterns)]
...
### Save best P (via chi-squared)
fixed_A = pd.DataFrame(model.fixed_A.detach().cpu(), columns = fixed_pattern_names, index=result_anndata.var.index)
#best_P.columns = ['Pattern_' + str(x+1) for x in best_P.columns]
#best_P.index = result_anndata.obs.index
result_anndata.varm['fixed_A'] = fixed_A
print("Saving fixed A via chi2 in anndata.obsm['fixed_A']")

### Save best P (via chi-squared)
best_A_total = fixed_A.merge(best_A, left_index=True, right_index=True)
result_anndata.varm['best_A_total'] = best_A_total
print("Saving best A via chi2 in anndata.obsm['best_A_total']")


### Save best locP (via chi-squared)
best_locP = pd.DataFrame(model.best_locP.detach().cpu().numpy())
best_locP.columns = fixed_pattern_names + ['Pattern_' + str(x+1) for x in np.arange(num_patterns)]
