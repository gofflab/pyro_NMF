...
pat = pd.DataFrame(data.obsm['spfac']).loc[:,[0]]
#layers = data.obsm['atlas'].loc[:,['SS','MO']]*1
#continous = pd.read_csv('/raid/kyla/projects/pyro_NMF/analyses/continuous_cortex/unroll_cortex.csv',index_col=0)
#coords = coords.loc[layers.index,:]
#data = data[layers.index,:]
...
#### output parameters
outputDir = '/raid/kyla/projects/pyro_NMF/analyses/nsf_spatialSimulated/SS'
savename = 'fix_p1' # output anndata will be saved in outputDir/savename.h5ad
