...
#from pyroNMF.models.gamma_NB_base_singleScaleFixed import Gamma_NegBinomial_base
from pyroNMF.models.gamma_NB_base_singleScaleFixed_uncertainty import Gamma_NegBinomial_base
import random
...
#S1235 = ad.read_h5ad('/mnt/morbo/Users/kwoyshner/octopus/data/fixed/20240311_S1235inner_transverse_pearson_residual_in_tissue.h5ad')
S1 = ad.read_h5ad('/mnt/morbo/Users/kwoyshner/octopus/data/fixed/20240311_S1_ventral_transverse_pearson_residual_in_tissue.h5ad')

#%%
# (14545, 12320)
data = S1
D = torch.tensor(data.layers['raw'].todense()) ## RAW COUNT DATA
...
#NB_probs = (D*0.1).clip(0.3,0.9) # in range [0,1]; if None: use default of 1 - sparsity; this is the probs argument for NegativeBinomial for D
uncertainty = (D*0.1).clip(min=0.3) # in range [0,1]; if None: use default of 1 - sparsity; this is the probs argument for NegativeBinomial for D
...
#### output parameters
outputDir = '/home/kyla/projects/pyro_NMF/results/octopus_S1'
savename = 'octopus_S1_wChisq_fixUnc_10kIter_run1' # output anndata will be saved in outputDir/savename.h5ad

useTensorboard = True
tensorboard_identifier = 'octopus_S1_wChisq_fixUnc_10kIter_run1' # key added to tensorboard output name

spatial = True # if True, will plot patterns on spatial coordinates

if spatial:
    #### coords should be two columns named x and y
    coords =  pd.DataFrame(data.obsm['X_spatial'],columns=['x','y'])
    plot_dims = [4, 3] # rows x columns should be > num patterns; this is for plotting
...
model = Gamma_NegBinomial_base(D.shape[0], D.shape[1], num_patterns, uncertainty=uncertainty, scale=scale, use_chisq=use_chisq, NB_probs = NB_probs, device=device)          
...
#result_anndata.uns['settings'] = settings
