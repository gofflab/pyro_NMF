...
data = ad.read_h5ad('/raid/kyla/data/DLPFC/151674.h5ad')
data.obsm['Spatial'] = data.obsm['spatial'] # samples x 2

learned_patterns = ad.read_h5ad('/raid/kyla/projects/pyro_NMF/analyses/fixLayersAreas/fixSpots_n20.h5ad')
fixed_geneWeights = learned_patterns.varm['best_A'].loc[:,['layer 1', 'layer 2/3', 'layer 4', 'layer 5', 'layer 6']]

#%%
'''
layers = data.obsm['atlas'].loc[:,['layer 1','layer 2/3','layer 4','layer 5', 'layer 6']]*1

# Store average expression per gene for each layer
avg_expr_dict = {}

# Loop over each layer
for layer in ['layer 1', 'layer 2/3', 'layer 4', 'layer 5', 'layer 6','SS','MO']:
#for layer in layers.columns:
    mask = data.obsm['atlas'][layer] == 1     # Get boolean index for spots in this layer
    adata_subset = data[mask]     # Subset AnnData
    avg_expr = adata_subset.X.mean(axis=0)     # Average gene expression across spots (axis=0)
    avg_expr_dict[layer] = pd.Series(avg_expr, index=data.var_names)     # Store with gene names as index
fixed_geneWeights = pd.DataFrame(avg_expr_dict)
'''
...
#### output parameters
outputDir = '/raid/kyla/projects/pyro_NMF/analyses/DLPFC'
savename = 'fixLayerGenesLearnedPyroNMF' # output anndata will be saved in outputDir/savename.h5ad
...
### Save best P (via chi-squared)
fixed_A = pd.DataFrame(model.fixed_A.detach().cpu().T, columns = fixed_pattern_names, index=result_anndata.obs.index)
#best_P.columns = ['Pattern_' + str(x+1) for x in best_P.columns]
#best_P.index = result_anndata.obs.index
